...
from .types import ReviewEmailGroup


async def render_commit(
    group: ReviewEmailGroup, commit: api.commit.Commit, context_lines: int
) -> Sequence[str]:
    cache: dict[Tuple[api.commit.Commit, int], list[str]] = group.cache.setdefault(
        "render_commit", {}
    )
